...
\documentclass[10pt,twoside]{article}
    ...
    \def\lt{<}
    % Document parameters
    \title{Synthesis of function generators cam-follower systems for rapid prototyping}
...
\makeatother
    ...
    \allsectionsfont{\bfseries\normalsize}
        ...
        \geometry{a4paper,includeheadfoot,tmargin=1.25cm,bmargin=1.25cm,lmargin=4.5cm,rmargin=3.5cm}
    

    \let\oldvrb\verbatim
    \renewenvironment{verbatim}{\small\begin{oldvrb}}{\end{oldvrb}}

    %\let\oldhigh\Highlighting
    %\renewenvironment{Highlighting}{\begin{oldhigh}}{\end{oldhigh}}
...
\vspace{1.2cm}
{\Large\textbf{Synthesis of function generators cam-follower systems for rapid prototyping}}\par
\vspace{0.6cm}
\begin{itshape}
\large{Candidato: Federico Ciotti
...
\bigskip
Lo scopo di questo progetto è lo sviluppo di un software open source per la progettazione dimeccanismi a camma
...
\newgeometry{includeheadfoot,tmargin=1.25cm,bmargin=1.25cm,lmargin=3cm,rmargin=2cm}
    ...
    The program interface is an interactive console written in \emph{Python
3} and it makes use of different libraries:
...
\texttt{-h} or \texttt{-\/-help} option. Avalaible command are listed
here:
...
discussed thoroughly in the following sections.
    ...
    \section{Follower travel generation}\label{follower-travel-generation}
...
The travel is generated from interpolation of points from an input file
(list of semicolon-semapared fraction of unit length and heigth) with a
given method or as discretization of a function between given bounds.
The command calls the method \emph{gen} of a persistent instance the
class \emph{Travel} which reads the input file or else parses the
function, then it calls \emph{update} which populates the class arrays
\emph{x} and \emph{y} calling the appropriate method from the
\emph{interpolation} module or discretizing the function.

    \section{Cam generation}\label{cam-generation}
...
a
    ...
    \begin{Shaded}
\begin{Highlighting}[]
\NormalTok{parser_update_cam.add_argument(}\StringTok{'--nonflat'}\NormalTok{, dest}\OperatorTok{=}\StringTok{'flat'}\NormalTok{, action}\OperatorTok{=}\StringTok{'store_const'}\NormalTok{, const}\OperatorTok{=}\VariableTok{False}\NormalTok{, }\BuiltInTok{help}\OperatorTok{=}\StringTok{'non flat follower'}\NormalTok{)   }
\end{Highlighting}
\end{Shaded}
